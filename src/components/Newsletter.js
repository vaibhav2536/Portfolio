...
const Newsletter = () => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setStatus({ success: true, message: "**Thank you for subscribing to my newsletter!**" });
    } catch (error) {
      console.log(error);
  ...
  return (
    ...
    >
      <h2 style={{ fontSize: "24px", fontWeight: "bold", marginBottom: "10px", color: "#333" }}>Subscribe To My Newsletter</h2>
      <form onSubmit={handleSubmit}>
        ...
        {status.message && <p style={{ color: "green", fontWeight: "bold", fontSize: "16px", marginTop: "10px" }}>{status.message}</p>}
      </form>
    </section>
  );
};
